...
// "tidak_ditemukan" -- opsi TAMBAHAN atas permintaan user: dipakai kalau
// petugas SUDAH ke lokasi tapi alamatnya sendiri tidak ketemu/tidak jelas
// (BEDA dari "Tidak Ada" yg berarti lokasi ketemu tapi usahanya memang
// tidak ada). Krn ketidakjelasan itu bukan salah usaha ybs, nilai ini
// SENGAJA diperlakukan sama dgn "Ada" di sisi server (dihitung sbg "sudah
// didata di SE2026" -- ikut mengisi ada_konfirmasi_ppl/ada_konfirmasi_jorong
// & masuk kolom "Ada" di Ringkasan Hasil Identifikasi, lihat
// app/api/penyisiran/identifikasi/route.ts & RPC
// penyisiran_ringkasan_identifikasi) supaya tidak keliru dianggap kasus
// undercoverage baru -- tapi tetap diberi label & warna SENDIRI di sini
// supaya kartu jujur soal APA yg sebenarnya terjadi di lapangan.
      ...
      {filterKec && topJorong.length > 0 && (
        <div className="rounded-lg border border-[#F4D77A] bg-[#FCEFD1] p-3">
          <p className="text-xs font-medium text-[#8A6A12] sm:text-sm">
            ...
            dengan jumlah potensi kasus terbanyak di kecamatan ini. Cek daftarnya REKOMENDASI SLS DENGAN JUMLAH KASUS
            TERBANYAK di bawah -- klik salah satu untuk langsung mengaktifkan filter ke Jorong tersebut.
          </p>
