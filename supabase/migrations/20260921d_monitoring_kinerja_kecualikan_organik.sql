-- Perluas filter "hanya PPL" di kartu #1 "Monitoring Penyisiran Sensus
-- Ekonomi 2026" (migrasi 20260921b_monitoring_kinerja_hanya_ppl.sql) --
-- user konfirmasi: SEMUA petugas berstatus_kepegawaian "organik" adalah
-- PML/pengelola (BUKAN petugas lapangan), termasuk yang TIDAK punya
-- bawahan sama sekali di pengawas_id (mis. akun pengelola khusus Master
-- Petugas/Manajemen Target spt Bambang, Deswaty, Arini yg memang tidak
-- mengawasi PPL manapun, & Yudi Firdian).
--
-- Filter lama (cuma "not exists bawahan") TIDAK cukup krn ada 2 org PML
-- asli (py bawahan) yg status_kepegawaian-nya NULL (bukan "organik", data
-- belum lengkap) -- makanya filter di sini DIGABUNG (OR), bukan diganti:
--  - status_kepegawaian = 'organik' -> dikecualikan (aturan baru, user
--    confirmed "keluarkan semua yang organik").
--  - ATAU py >=1 bawahan (pengawas_id menunjuk ke dia) -> dikecualikan
--    (aturan lama, jaring pengaman utk PML yg status_kepegawaian-nya belum
--    terisi/bukan "organik").
-- Hasilnya: 14 baris tersisa (SEMUA berstatus "mitra", petugas lapangan
-- murni) -- dicek manual via SQL sebelum migrasi ini diterapkan.
...
as $function$
  ...
  petugas as (
    ...
    where p.aktif
      and p.status_kepegawaian is distinct from 'organik'
      and not exists (
