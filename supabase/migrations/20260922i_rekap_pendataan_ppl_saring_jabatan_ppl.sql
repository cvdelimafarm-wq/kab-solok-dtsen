-- "📋 Rekap Pendataan PPL" (tab Perencanaan Lapangan) msh menampilkan
-- SEMUA petugas aktif (termasuk PML & kepala kantor), padahal judulnya
-- sendiri sudah "Rekap Pendataan PPL" -- PML/kepala kantor memang tidak
-- pernah mengisi alokasi/pendataan sendiri (baris mrk selalu 0 di semua
-- kolom), cuma bikin daftar lebih panjang & membingungkan. Permintaan
-- user: "baris yang ditampilkan masih seluruh petugas, harusnya hanya
-- baris PPL saja" -- tambah `and p.jabatan = 'ppl'` di WHERE utama.
-- pengawas_nama (join ke PML) TETAP ADA tidak diubah, cuma baris ROOT-nya
-- yg disaring. totalPpl/totalKk di FE (perencanaan-lapangan.tsx) dihitung
-- dari data.per_petugas, jadi otomatis ikut turun tanpa perubahan FE.
  ...
  select jsonb_build_object(
    'per_petugas', (
      select coalesce(jsonb_agg(x order by x.jumlah_subsls_ditag desc, x.nama), '[]'::jsonb) from (
        ...
        where p.aktif and p.jabatan = 'ppl'
      ) x
    ),
